...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains all elements that first set, but not in the second set") {
    new TestSets {
      val s = union(s1, s2)
      val d = diff(s, s1)
  ...
  test("filter return subset of set element that hold some criteria") {
    new TestSets {
  ...
  test("forall returns whether all bounded integers within set hold some criteria") {
    new TestSets {
      val s = union(union(s1, s2), s3)
  ...
  test("exists returns whether some integer(s) within set hold some criteria") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      assert(exists(s, x => x > 0), "exists 1")
      assert(exists(s, x => x > 1), "exists 2")
      assert(exists(s, x => x > 2), "exists 3")
      assert(!exists(s, x => x > 3), "exists 4")
    }
  }

  test("map returns set by applying the function to each element of input set") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val m = map(s, x => x * x)
